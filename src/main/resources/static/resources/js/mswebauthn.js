...
var webauthn = (function () {
    ...
    function msMakeCredential(accountInfo, cryptoParams, attestChallenge, options) {
    ...
    function msGetAssertion(challenge, allowlist, extensions) {
		...
		if (allowlist) { 
			for ( j = 0; j < allowlist.length; j++ ) {
				if ( allowlist[j].type === 'FIDO' ) {
					credList[j] = { type: 'FIDO_2_0', id: allowlist[j].id };
				} else {
					credList[j] = options.allowlist[j];
				}
			}
			filter = { accept: credList }; 
		}
		if (extensions && extensions['webauthn_txAuthSimple']) { sigParams = { userPrompt: extensions["webauthn_txAuthSimple"] }; }
